...
export default function WizardShell() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem('campaign-wizard-draft');
      if (raw) {
        ...
        if (d.editableBody) setEditableBody(d.editableBody);
        if (d.editableSubject) setEditableSubject(d.editableSubject);
        if (Array.isArray(d.selectedLists)) setSelectedLists(new Set(d.selectedLists));
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    try {
      sessionStorage.setItem('campaign-wizard-draft', JSON.stringify({
        campaignName, selectedTemplateId, accountId,
        editableBody, editableSubject,
        selectedLists: [...selectedLists],
      }));
    } catch {}
  }, [campaignName, selectedTemplateId, accountId, editableBody, editableSubject, selectedLists]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setAliases(als || {});
      setSignatures(sigs || {});
    }).catch((err: any) => setError(err.message || 'Failed to load wizard data'));
  }, [authed]);
  ...
  const handleSend = async (scheduledAt: string | null, delayMinMs?: number, delayMaxMs?: number) => {
    ...
    try {
      ...
      sessionStorage.removeItem('campaign-wizard-draft');
