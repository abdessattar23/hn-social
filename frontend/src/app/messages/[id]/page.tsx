...
export default function MessageDetailPage() {
  ...
  useEffect(() => {
    ...
    api.get('/messages').then((msgs: Message[]) => {
      ...
      setAllTags([...set].sort());
    }).catch((err: any) => setLoadError(err.message || 'Failed to load tags'));
  }, [authed, id]);

  const load = () =>
    api.get(`/messages/${id}`).then((m) => {
      setMsg(m); setName(m.name); setSubject(m.subject || ''); setBody(m.body);
    }).catch((err: any) => setLoadError(err.message || 'Failed to load template'));
  ...
  return (
    <AnimatedPage>
      ...
      {loadError && (
        <div className="bg-red-light-6 border border-red/20 text-red rounded-xl px-4 py-3 text-sm mb-3 flex items-center justify-between">
          <span>{loadError}</span>
