...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLoaded(true);
    }).catch((err: any) => { setLoadError(err.message || 'Failed to load dashboard data'); setLoaded(true); });
  }, [authed]);
  ...
  return (
    <AnimatedPage className="space-y-8">
      ...
      {loadError && (
        <div className="bg-red-light-6 border border-red/20 text-red rounded-xl px-4 py-3 text-sm flex items-center justify-between">
          <span>{loadError}</span>
